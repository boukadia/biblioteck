...
export class RegleSanctionsService {
  ...
  async create(data: CreateRegleSanctionDto, user: JwtUser) {
    if (user.role !== RoleUtilisateur.ADMIN) {
      throw new UnauthorizedException(
        "Vous n'avez pas le droit de créer une règle de sanction",
      );
    }
    ...
    if (existing) {
      throw new BadRequestException(
        `Une règle avec le nom "${data.nomRegle}" existe déjà`,
      );
    }
  ...
  async findOne(id: number, user: JwtUser) {
    ...
    if (!regle) {
      throw new NotFoundException(
        `Règle de sanction avec l'ID ${id} introuvable`,
      );
    }
  ...
  async update(id: number, data: UpdateRegleSanctionDto, user: JwtUser) {
    if (user.role !== RoleUtilisateur.ADMIN) {
      throw new UnauthorizedException(
        "Vous n'avez pas le droit de créer une règle de sanction",
      );
  ...
  async remove(id: number, user: JwtUser) {
    ...
    return {
      message: `Règle de sanction "${regle.nomRegle}" supprimée avec succès`,
    };
  }
}
